...
Opiskelijanumero: 150350040
Tekijä: Aleksi Pellikka
Sähköposti: aleksi.pellikka@tuni.fi

Tehtävä 12.5: Muuttolaatikot olio-projekti
"""
...
class MovingBox:
    ...
    def __init__(self, box_name):
        """Constructor of the MovingBox-object
        :param box_name: String, name of the box
        :self.__contents_of_box: dictionary, to hold the contents of the
        box object in memory.
        """

        self.__box_name = box_name
        self.__contents_of_box = {}

    def add_item(self, item_name, item_count):
        """Adds an item to the box
        :param item_name: String, name of the item
        :param item_count: int, amount of items to be added to the box
        """

        if item_count < 1:
            return

        if item_name not in self.__contents_of_box:
            self.__contents_of_box[item_name] = item_count

        elif item_name in self.__contents_of_box:
            self.__contents_of_box[item_name] += item_count

    def list_content(self):
        """Prints the content of the box, both the name and amount of
        items.
        """

        total_items = 0
        for item in self.__contents_of_box:
            total_items += self.__contents_of_box[item]

        print(f"Box \"{self.__box_name}\" contains {total_items}"
              f" items.")

        if len(self.__contents_of_box) == 0:
            return

        # Styles the printing according to amount of items as required by
        # automatic tests.
        for item in sorted(self.__contents_of_box):
            if self.__contents_of_box[item] > 99:
                print(self.__contents_of_box[item], item)
            elif self.__contents_of_box[item] > 9:
                print("", self.__contents_of_box[item], item)
            else:
                print(" ", self.__contents_of_box[item], item)
    ...
    def remove_item(self, item_name, item_count):
        ...
        :param item_name: String, name of the item
        :param item_count: Int, the amount of items to be removed
        :return: Boolean, True if deletion succeeds, False if not.
        """

        if item_count < 1:
            return

        if item_name not in self.__contents_of_box.keys() or \
                item_count > self.__contents_of_box[item_name]:
            return False

        if item_name in self.__contents_of_box:
            self.__contents_of_box[item_name] -= item_count

            if self.__contents_of_box[item_name] <= 0:
                del self.__contents_of_box[item_name]

            return True

    def transfer_item(self, target_name, item_name, item_count):
        """Transfers items from one box to another. Deleting the correct
        amount from the source and adding the correct amount to the target
        :param target_name: String, name of the target box
        :param item_name: String, name of the item to be transferred
        :param item_count: Int, how many items of this particular kind
        is to be transferred.
        :return: boolean, true if transfer is successful, False if not.
        """

        if item_name in self.__contents_of_box:

            if self.__contents_of_box[item_name] < item_count:
                return False
    ...
    def search_item(self, item_name):
        """Searches how many items of a given item_name is in the box
        :param item_name: String, name of the item
        :return: Int, 0 if item is not in the box and the amount of items
        in the box if more than 0.
        """
        ...
        for item in self.__contents_of_box:
            if item == item_name:
                return int(self.__contents_of_box[item])
